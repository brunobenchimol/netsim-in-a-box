...
func TcInit(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
	...
	*/
	...
	if err := json.NewEncoder(w).Encode(response); err != nil {
		logger.Ef(ctx, "Failed to encode TcInit response: %v", err)
	}
...
func (v *NetworkOptions) Execute(ctx context.Context) error {
	...
	if v.direction == "outgoing" {
		...
		// PATCHED: Refactored to switch for linter (QF1003)
	...
	if v.direction == "incoming" {
		if !hasIFB {
			return errors.Errorf("Cannot apply 'incoming' rules: Kernel module 'ifb' is not loaded on the host.")
		}
		args = append(args,
			"--direction", "incoming",
...
type TcpdumpInterfaceSummary struct {
	...
	// PATCHED: Linter fix - Removed duplicate/unexported json tag.
	endpoints map[string]*TcpdumpEndpoint
}
...
func queryIPNetInterfaces(filter func(iface *net.Interface, addr net.Addr) bool) ([]*TcInterface, error) {
	...
	logger.Tf(nil, "Found %d total system interfaces. Filtering...", len(ifaces))

	for _, iface := range ifaces {
		logger.Tf(nil, "Inspecting interface: %s, Flags: %v", iface.Name, iface.Flags.String())
		if (iface.Flags & net.FlagPointToPoint) == net.FlagPointToPoint {
			continue
		}
		// --- Filter out loopback and down interfaces ---
		if (iface.Flags & net.FlagUp) == 0 {
			logger.Tf(nil, "Skipping %s: Interface is down", iface.Name)
			continue // Interface is down
		}
		if (iface.Flags & net.FlagLoopback) != 0 {
			logger.Tf(nil, "Skipping %s: Interface is loopback", iface.Name)
		...
		logger.Tf(nil, "  - Found %d addresses for %s", len(addrs), iface.Name)

		ti := &TcInterface{Name: iface.Name}
		for _, addr := range addrs {
			logger.Tf(nil, "    - Inspecting addr: %v", addr.String())
		...
		if ti.IPv4 != nil || ti.IPv6 != nil {
			...
			logger.Tf(nil, "  - SUCCESS: Added %s to list", iface.Name)
		} else {
			logger.Tf(nil, "  - FAILED: No valid IPv4/IPv6 found for %s, skipping", iface.Name)
		}
	}
